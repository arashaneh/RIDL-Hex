...
#from dic_learning_rotUpdate import sq_rot_invar, sq_dict_learning, sec_rot_invar #, rotate_filters, select_correlated_orientation
from dic_learning_LMS import sq_rot_invar, sq_dict_learning, sec_rot_invar #, rotate_filters, select_correlated_orientation
...
if __name__ == '__main__':
    ...
    t0 = time()
    D1, losses1 =  sec_rot_invar(data, n_filters = n_filter, n_theta = 6, eta = 1e-1, # D_0 = D_0,
    ...
    D_0 = data[sample_ids, :].T
    t0 = time()
    D2, losses2 =  sec_rot_invar(data, n_filters = n_filter, n_theta = 6, eta = 1e-1, # D_0 = D_0,
                                 n_sections = 6, sparsity = sparsity, n_epochs = n_epochs, EV_SCORE = EV_SCORE)
    dt = time() - t0
    ...
    data = X_sq - np.expand_dims(np.mean(X_sq, axis=1), 1)*dim_ratio
    data *= np.expand_dims(mask.flatten(),0)
    data /= np.expand_dims(np.linalg.norm(data, axis=1), axis=1)
    D_0 = data[sample_ids, :].T
    t0 = time()
    D3, losses3 = sq_rot_invar(data, mask,  n_filters = n_filter, n_theta = 6, eta = 1e-1, #D_0 = D_0,
                                sparsity = sparsity, n_epochs = n_epochs, EV_SCORE = EV_SCORE) # rho =  0.1
    dt = time() - t0
    print 'time needed for Square Sampling: ' + str(dt)                          
  
   
    
    D_0 = data[sample_ids, :].T
    t0 = time()
    D4, losses4 = sq_dict_learning(data, mask,  n_filters = n_filter, eta = 1e-1, #D_0 = D_0,
    ...
    D_0 = data[sample_ids, :].T
    t0 = time()
    D5, losses5 = sq_dict_learning(data, mask,  n_filters = n_filter*6, eta = 1e-1, #D_0 = D_0,
                                 sparsity = sparsity, n_epochs = n_epochs, EV_SCORE = EV_SCORE)
    dt = time() - t0
    print 'time needed for Square Sampling (old): ' + str(dt)      
    ...
    plt.plot(iterations,losses3*mask.sum()/mask.shape[0]/mask.shape[1],'--k',linewidth=2)
    ...
    'Square Grid (Rot_Invar, 80)','Square Grid (NRot_Invar, 80)', 'Square (NRot_Invar, 480)'],fontsize=14)
    plt.xlabel('Iteration',fontsize=18)
    plt.ylabel('MSE',fontsize=18)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.title('Gradient Descent and Projection')
